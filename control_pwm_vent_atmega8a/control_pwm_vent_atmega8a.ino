...
void calib_vent() {                                                   //калибровка вентиляторов
  ...
  for (byte i = 255; i >= 0; i -= cc_step) {                                  //в цикле снижаем pwm.
    ...
    if ((rpm1 < rpm_min1) && !fake_rpm1 && (rpm1 != 0)) {              //вент замедляется, флага бреда тахометра не было, и вент не стоит (главный цикл замера rpm_min и pwm_up)
      ...
      if ((drpm1 > drpm1_old) && !flag_rpm1_up) {                    //девиации оборотов составляют постоянное значение(?) +-1(*60), при дельте оборотов больше этого значения, предполагаем начала устойчивого снижения - началась реакция вента на изменение pwm
        flag_rpm1_up = true;
        pwm1_up=i;
      }
    }
    ...
    //        if (!flag_rpm11_up) {
    //          flag_rpm11_up = true;
    //          pwm1_up = i;                                                 //верхняя граница pwm при которой начинает реагировать вент - снижение скорости второй раз, а значит гарантированно снижаются обороты. первым снижением пренебрегаем, т.к. есть девиация показаний
    //        }
    //      }
    //      rpm_min1 = rpm1;
    //      pwm_min1 = i;
    ...
    if ((rpm2 < rpm_min2) && !fake_rpm2 && rpm2 != 0) {
      if (!flag_rpm2_up) flag_rpm2_up = true; else {
        if (!flag_rpm22_up) {
          flag_rpm22_up = true;
          pwm2_up = i;                                                 //верхняя граница pwm при которой начинает реагировать вент - снижение скорости второй раз, а значит гарантированно снижаются обороты. первым снижением пренебрегаем, т.к. есть девиация показаний
        }
      }
      rpm_min2 = rpm2;
      pwm_min2 = i;
    ...
    if ((rpm3 < rpm_min3) && !fake_rpm3 && rpm3 != 0) {
      if (!flag_rpm3_up) flag_rpm3_up = true; else {
        if (!flag_rpm33_up) {
          flag_rpm33_up = true;
          pwm3_up = i;                                                 //верхняя граница pwm при которой начинает реагировать вент - снижение скорости второй раз, а значит гарантированно снижаются обороты. первым снижением пренебрегаем, т.к. есть девиация показаний
        }
      }
      rpm_min3 = rpm3;
      pwm_min3 = i;
